...
namespace ChatServer
{
    class Program
    {
        ...
        {
            var server = new TcpChatServer(IPAddress.Loopback, 5000);
            await server.StartAsync();
        }
    }

    public record Packet(string type, JsonElement payload);
    ...
    public class TcpChatServer
    {
        ...
        private readonly Dictionary<string, string> _creds = new() { ["alice"] = "pass1", ["bob"] = "pass2", ["carol"] = "pass3" };
        ...
        private async Task HandleClientAsync(TcpClient tcp)
        {
            ...
            string username = pkt.payload.GetProperty("username").GetString();
            string password = pkt.payload.GetProperty("password").GetString();
            ...
            _ = Task.Run(() => session.SendLoopAsync(_cts.Token));
                ...
                while (true)
                {
                    ...
                    switch (pkt.type.ToUpperInvariant())
                    {
                        case "DM":
                            ...
                            if (_sessions.TryGetValue(to, out var dest))
                            {
                                dest.Enqueue(MakeEnvelope("INCOMING_DM", new { from = session.Username, msg }));
                        ...
                        case "MULTI":
                            ...
                            foreach (var t in toArr)
                            {
                                ...
                                if (_sessions.TryGetValue(destUser, out var destS))
                                {
                                    destS.Enqueue(MakeEnvelope("INCOMING_MULTI", new { from = session.Username, msg = multiMsg }));
                                    Interlocked.Increment(ref _messagesDelivered);
                                    _deliveredTimestamps.Enqueue(DateTime.UtcNow.Ticks);
                                    AppendAudit(now, session.Username, destUser, "MULTI", Encoding.UTF8.GetByteCount(multiMsg ?? ""));
                                }
                            }
                            break;
                        case "BROADCAST":
                            ...
                            foreach (var kv in _sessions)
                            {
                                ...
                                kv.Value.Enqueue(MakeEnvelope("INCOMING_BROADCAST", new { from = session.Username, msg = broadcastMsg }));
                                Interlocked.Increment(ref _messagesDelivered);
                                _deliveredTimestamps.Enqueue(DateTime.UtcNow.Ticks);
                                AppendAudit(now, session.Username, kv.Key, "BROADCAST", Encoding.UTF8.GetByteCount(broadcastMsg ?? ""));
                            }
                            break;
                        case "HEARTBEAT":
                            break; // refresh already done via UpdateActivity()
                    }
                }
            }
            catch (IOException) { }
        ...
        {
            if (_sessions.TryRemove(username, out var s)) { Console.WriteLine($"{username} disconnected"); s.Close(); }
        ...
        private async Task StatsLoop()
        {
            while (!_cts.IsCancellationRequested)
            {
                ...
                lock (_latLock) { if (_latencies.Count > 0) { min = _latencies.Min(); max = _latencies.Max(); avg = (long)_latencies.Average(); if (_latencies.Count > 1000) _latencies.RemoveRange(0, _latencies.Count - 1000); } }
                =Console.WriteLine($"[STAT] online={_sessions.Count} totalRecv={_messagesReceived} totalDelivered={_messagesDelivered} msgs/sec={recent} latency_us(min/avg/max)={min}/{avg}/{max}");
    ...
    public class ClientSession
    {
        ...
        public async Task SendLoopAsync(CancellationToken ct) { try { foreach (var msg in _sendQueue.GetConsumingEnumerable(ct)) { await Writer.WriteLineAsync(msg); } } catch (OperationCanceledException) { } }
